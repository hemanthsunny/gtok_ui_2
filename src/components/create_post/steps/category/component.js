...
const DetailComponent = ({
  ...
  const [otherValue, setOtherValue] = useState(
    category.key === "other" && category.title
  );
          ...
          <div className="modal-body pt-0">
              ...
              {postCategories.map((obj, idx) => (
                <div
                  className={`post-category ${
                    obj.key === "special" && "text-color-special"
                  }`}
                  key={idx}
            ...
            <div className={`${other ? "other-category-box" : "d-none"}`}>
              ...
              <small className="pull-right">
                {15 - otherValue.length} chars left
              </small>
            </div>
            <br />
            {errorMsg && (
              <div className="text-danger font-small my-2">{errorMsg}</div>
            )}
            <div className="text-center">
              <button

...
function AddPaymentCardComponent({currentUser}) {
  ...
  const saveCard = async () => {
    ...
    let result = await add("paymentCards", data);
    /* Log the activity */
  	// await add("logs", {
      ...
      <HeaderComponent save={saveCard} />
        ...
        <div className="form-group row">
          ...
          <div className="col-12 row">
            ...
            <div className="col-4">
              <input type="number" className="form-control" id="cardExpiryDate" value={expiryYear} placeholder="YY" onChange={e => handleChange('expiryYear', e.target.value)} maxLength="2" required />
            </div>
          </div>
        </div>
        <div className="form-group row">

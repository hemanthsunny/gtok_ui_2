...
function InviteFriendsComponent({ currentUser }) {
  ...
  useEffect(async () => {
    const i = await getQuery(
      firestore
        .collection("referals")
        .where("invitedBy", "==", currentUser.username)
        .get()
    );
    setInvites(i.length);
  }, []);
  ...
  const copyLink = () => {
    navigator.clipboard.writeText(
      `https://localhost:3000/signup/${currentUser.username}`
    );
    setCopyText(true);
    setTimeout(() => {
      setCopyText(false);
    ...
    <div>
      ...
      <div className="dashboard-content -xs-bg-none">
        <div className="invite-friends-wrapper text-center">
          <input
            type="text"
            className="lg-input col-9"
            value={inviteLink}
            disabled
          />
          <div className="my-3">
            {copyText ? (
              <div className="text-violet">Link is copied</div>
            ) : (

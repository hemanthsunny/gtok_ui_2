            ...
            {subHeader()}
            {
              posts[0] && posts.map((post, idx) => !post.premium && (
                <div className='card post-card-wrapper' key={idx}>
                  <div>
                    <span className='card-badge mr-2'>{moment(post.createdAt).format('DD/MM/YY HH:mm')}</span>
                    <span className='card-badge' onClick={e => window.alert('Login to see who created this post.')}>@Anonymous</span>
                  </div>
                  <div className='card-body pt-2'>
                    ...
                    <div className='edit-options float-right'>
                      <button className='btn btn-link' onClick={e => window.alert('Login to share the post.')}>

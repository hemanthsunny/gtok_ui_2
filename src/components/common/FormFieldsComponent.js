...
const FormFieldsComponent = ({ ques, response, setResponse }) => {
  ...
  const typeGeneral = () => (
    <input
      type={ques.type}
      className="form-control survey-input"
      id="typeGeneral"
      placeholder="Your response here..."
      required={ques.required === "true"}
      onChange={(e) =>
        setResponse({ ...response, [ques.question]: e.target.value })
      }
    />
  );
  const typeFile = () => (
      ...
      {ques.value.map((val, idx) => (
        <div className="form-check col-xs-12 col-sm-6 col-md-4" key={idx}>
          <input
            ...
            id={ques.qId + "." + idx}
            value={val}
            onChange={(e) => setArrayValues(ques.question, e.target.value, idx)}
          />
          <label className="form-check-label" htmlFor={ques.qId + "." + idx}>
            {val || (
              <div className="d-flex flex-row align-items-center">
  ...
  const typeDropdown = () => (
    <div>
      ...
      >
        ...
        {ques.value.map((val, idx) => (
          <option value={val || "Other"} key={idx}>
            {val || "Other"}
          </option>
        ))}
      </select>
  ...
  return (
    <div className="form-group">
      <label htmlFor={ques.qId} className="survey-question mb-0">
        {ques.question}{" "}
        <span
          className={`text-danger ${ques.required !== "true" && "d-none"}`}
          title="Required"
        >
          *
        </span>

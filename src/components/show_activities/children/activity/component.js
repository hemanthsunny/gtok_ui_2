...
const Component = ({ currentUser, activity, purchaseOrders, allUsers }) => {
  ...
  const deleteActivity = async (activity) => {
    if (
      activity.id &&
      window.confirm("Are you sure to delete this activity?")
    ) {
      ...
      <div className="card activity-card-wrapper">
        ...
        <div>
          <span className="card-badge mr-2">
            {moment(activity.createdAt).format("DD/MM/YY HH:mm")}
          </span>
          <span className="card-badge mr-2">
            {activity.anonymous ? (
              <span>@Anonymous</span>
            ) : (
              <span className="pointer" onClick={(e) => redirectToProfile()}>
                @{postedUser.username}
              </span>
            )}
          </span>
        ...
        currentUser.id !== activity.userId ? (
          <div className="card-body">
            ...
            <Link
              to={`/app/profile/${activity.userId}/unlock_profile`}
          ...
          <div className="card-body">
            <div>
              <div className="white-space-preline">{activity.activity}</div>
          ...
          <div className="media-body">
            <div className="d-none flex-inline align-items-center float-left">
              ...
              <div className={`${!showComments && "d-none"}`}>
                <button className="btn btn-violet-outline btn-sm mx-2 px-2">
                  Admire you
                </button>
                <button className="btn btn-violet-outline btn-sm mx-2 px-2">
            ...
            <div className="edit-options float-right">
              <button
                className={`btn btn-link ${
                  activity.userId !== currentUser.id && "d-none"
                }`}

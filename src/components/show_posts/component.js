  ...
  checkRelationships = (posts) => {
    setTimeout(() => {
      if (this.props.relations) {
        const rlns = this.props.relations.filter(
          (rln) =>
            rln.userIdOne === this.props.currentUser.id && rln.status === 1
        );
  ...
  checkFilters = (posts) => {
    if (this.state.selectedFilters[0]) {
      posts = posts.filter(
        (p) =>
          p.category &&
          this.state.selectedFilters.indexOf(p.category.title) > -1
      );
    }
    return posts;
  };
    ...
    if (
      ...
      let posts = await getQuery(
        firestore
          ...
          .where("active", "==", true)
          .orderBy("createdAt", "desc")
          .limit(this.state.pageId * this.state.pageLimit)
          .get()
      );
      posts = await this.checkRelationships(posts);
      posts = await this.checkFilters(posts);
    ...
    if (action === "selected") {
      this.setState({
        selectedFilters: Array.from(
          new Set([...this.state.selectedFilters, val])
        ).sort(),
        unselectedFilters: this.state.unselectedFilters
          .filter((f) => f !== val)
          .sort(),
      });
    } else {
  ...
  subHeader = () => (
    <div className="dashboard-tabs" role="navigation" aria-label="Main">
      <div className="tabs -big">
        <Link to="/app/assets" className="tab-item -active">
          Feelings
        </Link>
        <Link to="/app/activities" className="tab-item">
          Activities
        </Link>
      </div>
    </div>
      ...
      <div>
        ...
        <div>
          ...
          >
              ...
              <div className="filter-wrapper d-none">
                ...
                >
                  {this.state.selectedFilters.map((name, i) => (
                    <div
                      className="btn btn-violet-outline btn-sm mx-1 selected"
                      key={i}
                      onClick={(e) => this.handleFilters("unselected", name)}
                    >
                      {name}
                    </div>
                  ))}
                  {this.state.unselectedFilters.map((name, i) => (
                    <div
                      className="btn btn-sm mx-1 unselected"
                      key={i}
                      onClick={(e) => this.handleFilters("selected", name)}
                    >
                      {name}
                    </div>
                  ))}
                  ...
                  if (post.resharePostId) {
                    return (
                      <ResharePostComponent
                        ...
                        handleResharePostUser={this.handleResharePostUser}
                      />
                    );
                  }
                  return (
            ...
            <ShareOptionsComponent
              currentUser={this.props.currentUser}
              resharePostUser={this.state.resharePostUser}
            />

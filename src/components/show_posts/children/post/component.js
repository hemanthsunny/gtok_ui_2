...
const PostComponent = ({
  ...
  const [follower, setFollower] = useState(
    !!displayPost.followers.find((f) => f === currentUser.id)
  );
  ...
  const trans = transactions.find(
    (trans) =>
      trans.userId === currentUser.id && trans.postId === displayPost.id
  );
  ...
  const followPost = async (e) => {
    ...
    if (!follower) {
      ...
      await update("posts", displayPost.id, {
        followers: arrayAdd(currentUser.id),
        followersCount: displayPost.followers.length + 1,
      });
      if (currentUser.id !== postedUser.id) {
        /* Log the activity */
        await add("logs", {
  ...
  const sharePost = async (opt) => {
    if (opt === "shareOptions") {
      localStorage.setItem(
        "sharePostText",
        `Hey! I want to share this asset with you. Have a look ${process.env.REACT_APP_URL}app/posts/${post.id}`
      );
    }
  ...
  return (
    ...
    displayPost.stories && (
      <div className={`d-flex ${reshare ? "m-0" : "ml-2 mt-3 mb-4"}`}>
        ...
        <div
          className={`card post-card-wrapper ${
            reshare ? "reshare-box" : "add-filter"
          } ${!displayPost.active && "hidden"} `}
        >
          {result.status && (
            <NotificationComponent result={result} setResult={setResult} />
          )}
            ...
            <div key={idx}>
              <div className={`card-body ${idx !== activeIndex && "d-none"}`}>
                <div>
                  {!post.resharePostId && (
                    <span
                      className={`${reshare ? "pull-left mr-2" : "d-none"}`}
                    >
                      {displayPost.anonymous ? (
                        <CustomImageComponent user={postedUser} size="sm" />
                      ) : (
                  ...
                  {displayPost.category ? (
                    <span
                      className={`card-badge ${
                        displayPost.category.key === "special" &&
                        "card-badge-special"
                      } ${!displayPost.active && "hidden"}`}
                      onClick={(e) =>
                        handleFilters &&
                        handleFilters("selected", displayPost.category.title)
                      }
                    >
                    ...
                    <span
                      ...
                      onClick={(e) =>
                        handleFilters && handleFilters("selected", "same pinch")
                      }
                    >
                      same pinch
                    </span>
                  ...
                  <span className="created-at">
                    {moment(displayPost.createdAt).format("h:mm a")} &middot;{" "}
                    {hideCurrentYear(displayPost.createdAt)}
                  </span>
                </div>
                ...
                currentUser.id !== displayPost.userId ? (
                  <div className="card-body hidden-post px-0">
                    ...
                    <div
                      className="locked-post pointer"
                      onClick={(e) => tradePostAction(displayPost.id)}
                    >
                      <div className="locked-post-text">
                        ...
                        <span className="currency-text">
                          <img
                            className="currency-icon"
                            src={
                              require("assets/svgs/currency/inr_violet.svg")
                                .default
                            }
                            alt="1"
                          />
                      ...
                      <div>
                        <img
                          src={require("assets/svgs/LockedPost.svg").default}
                  ...
                  <div>
                    <p className="card-text white-space-preline">
                      {story.text.length <= 150 || displayFullStory ? (
                        ...
                        <span
                          className="pointer"
                          onClick={(e) =>
                            setDisplayFullStory(!displayFullStory)
                          }
                        >
                          {convertTextToLink(story.text.slice(0, 149))}{" "}
                    ...
                    {displayPost.stories.length > 1 && (
                      <div className="carousel-effect">
                        <span
                          className="prev"
                          onClick={(e) => slideTo("prev", idx)}
                        >
                          Prev
                        </span>
                        <span
                          className="next"
                          onClick={(e) => slideTo("next", idx)}
                        >
                          Next
                        </span>
                      </div>
              ...
              <div className="card-footer">
                {displayPost.anonymous ? (
                  ...
                  <span className="author pointer" onClick={redirectToProfile}>
                    @{postedUser.username}
                  </span>
                )}
                <div className={`edit-options ${hideEditOptions && "d-none"}`}>
                  <button
                    className="btn btn-link btn-heart pr-0"
                    onClick={(e) => followPost(e)}
                  >
                    {follower ? (
                      <img
                        className={`icon-heart icon-heart-${displayPost.id}`}
                        src={require("assets/svgs/PinchActive.svg").default}
                        alt="1"
                      />
                    ) : (
                      <img
                        className={`icon-heart icon-heart-${displayPost.id}`}
                        src={require("assets/svgs/Pinch.svg").default}
                        alt="1"
                      />
                    )}
                  </button>
                  <button
                    ...
                    data-toggle="modal"
                    data-target="#shareOptionsModal"
                    onClick={(e) => sharePost("shareOptions")}
                  >
                    <img
                      className="icon-share"
                      src={require("assets/svgs/ShareBtn.svg").default}
                      alt="1"
                    />
                  </button>
                  <button
                    ...
                    onClick={(e) => sharePost("menuOptions")}
                  >
                    <img
                      className="icon-more"
                      src={require("assets/svgs/ShowMore.svg").default}
                      alt="1"
                    />
                  </button>
                </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    bindSharePost: (content, type, data) =>
      dispatch(SetSharePost(content, type, data)),
    bindUpdatedPost: (content, type, data) =>
      dispatch(SetUpdatedPost(content, type, data)),
  };
};

            ...
            <div className="d-flex flex-row align-items-center justify-content-between">
              <span
                className={`card-badge ${
                  resharePost.category.key === "special" && "card-badge-special"
                } ${!post.active && "hidden"}`}
                onClick={(e) =>
                  handleFilters("selected", resharePost.category.title)
                }
              >
                {resharePost.category && resharePost.category.title}
              </span>
              <div className="created-at">
                {moment(resharePost.createdAt).format("h:mm a")} &middot;{" "}
                {hideCurrentYear(resharePost.createdAt)}
              </div>
            </div>
            <div className="mt-2">{resharePost.stories[0].text}</div>
